...
let getInforATable = (data) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            // console.log("get a table")
            const connection = await mysql.createConnection(dbConfig);
            const [totalRows] = await connection.execute(`SELECT COUNT(*) AS total_records FROM ${nameDB}.${nameTable}`);
            const [columnTotal] = await connection.execute(`SELECT SUM(username) AS column_total ${nameDB}.${nameTable}`);
